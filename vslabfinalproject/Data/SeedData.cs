...
public static class SeedData
{
    public static async Task SeedAsync(AppDbContext db)
    {
        ...
        db.StudyTasks.AddRange(
            new StudyTask { StudentId = students[0].Id, Title = "Prepare Visual Programming viva", Description = "Revise Blazor pages, services, EF Core flow and UML explanation.", DueDate = DateTime.Today.AddDays(3), Priority = "High", Status = "Pending" },
            new StudyTask { StudentId = students[0].Id, Title = "Complete assignment screenshot folder", Description = "Capture annotated screenshots for login, dashboard, profile, fees, support and responsive views.", DueDate = DateTime.Today.AddDays(2), Priority = "Medium", Status = "Pending" },
            new StudyTask { StudentId = students[1].Id, Title = "Revise database relationships", Description = "Practice explaining Student, Course, Enrollment and FeeInvoice relationships.", DueDate = DateTime.Today.AddDays(5), Priority = "Medium", Status = "Pending" },
            new StudyTask { StudentId = students[2].Id, Title = "Submit support query follow-up", Description = "Check whether the existing support ticket has been resolved.", DueDate = DateTime.Today.AddDays(1), Priority = "Low", Status = "Completed", IsCompleted = true });
